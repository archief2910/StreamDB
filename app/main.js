...
const {lowerBound, upperBound,UpperBound} = require("./search.js");
...
const server = net.createServer((connection) => {
  ...
  connection.isMultiMode = false;
  connection.multiBuffer = [];
...
console.log(`${successfulReplicas}`)
      ...
      let ans=[];
      if(commandturn!==""){let sizer=commandturn.length; sizer-=10;
...
if(lastrange==command[10 + (2 * i) + (sizer / 2)]){res1.push(null);}
  ...
  connection.write(serializeRESP(res1));
    ...
    }else if(command[2]=="MULTI"){
      connection.isMultiMode=true;
      connection.write("+OK\r\n");
    }else if(command[2]=="DISCARD"){
    ...
    if(command[2]=="EXEC"){
      if (connection.isMultiMode) {
        ...
        let formattedString = `*${ans.length}\r\n` + ans.map(item => `${item}\r\n`).join("");
        // After processing all messages
        connection.isMultiMode = false; // Exit MULTI mode
        connection.multiBuffer = []; // Clear the buffer
        connection.write(formattedString); // Conve
