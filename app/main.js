...
const server = net.createServer((connection) => {
 ...
 if(replicaidx!==-1){
 // If it's a replica, connect to the master server
 ...
 const masterConnection = net.createConnection({ host: masterarray[0], port: masterarray[1] }, () => {
   ...
   // Do not respond to clients, only listen for updates from the master
   connection.on("data", () => {
    const command = Buffer.from(data).toString().split("\r\n");
 ...
 // On replica disconnect from the master, stop forwarding updates.
